...
module CloudConductorUtils
  class Consul
    ...
    def self.read_parameters
      begin
        response = RestClient.get("#{CONSUL_KVS_PARAMETERS_URL}?token=#{ENV['CONSUL_SECRET_KEY']}")
    ...
    def self.update_parameters(parameters)
      RestClient.put("#{CONSUL_KVS_PARAMETERS_URL}?token=#{ENV['CONSUL_SECRET_KEY']}", parameters.to_json)
    end

    def self.read_servers
      begin
        servers = {}
        response = RestClient.get("#{CONSUL_KVS_SERVERS_URL}?recurse&token=#{ENV['CONSUL_SECRET_KEY']}")
    ...
    def self.update_servers(hostname, server_info)
      RestClient.put("#{CONSUL_KVS_SERVERS_URL}/#{hostname}?token=#{ENV['CONSUL_SECRET_KEY']}", server_info.to_json)
    end
  end
end
