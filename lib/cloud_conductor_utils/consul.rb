...
module CloudConductorUtils
  class Consul
    ...
    def self.read_parameters
      begin
        response = RestClient.get("#{CONSUL_KVS_PARAMETERS_URL}?#{acl_token}")
    ...
    def self.update_parameters(parameters)
      RestClient.put("#{CONSUL_KVS_PARAMETERS_URL}?#{acl_token}", parameters.to_json)
    end

    def self.read_servers
      begin
        servers = {}
        response = RestClient.get("#{CONSUL_KVS_SERVERS_URL}?recurse&#{acl_token}")
    ...
    def self.update_servers(hostname, server_info)
      RestClient.put("#{CONSUL_KVS_SERVERS_URL}/#{hostname}?#{acl_token}", server_info.to_json)
    end

    def self.acl_token
      RestClient::Payload::UrlEncoded.new 'token' => "#{ENV['CONSUL_SECRET_KEY']}"
    end
  end
end
